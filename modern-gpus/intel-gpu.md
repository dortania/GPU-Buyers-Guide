...
Regarding the HD 4000, it's completely native with Catalina (though no longer supported with macOS Big Sur). The HD 2500, on the other hand, only has partial support in Mojave for quick sync features, but hardware acceleration is unsupported, so you will need a compatible dGPU for display purposes.
